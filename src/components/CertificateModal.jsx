...
export const CertificateModal = ({ isOpen, onClose }) => {
  ...
  const printCertificate = () => {
    ...
    // Certificate HTML with its own watermark
    const html = `
      <div style="position: relative; text-align: center; border: 12px double #2f86eb; padding: 40px; background: #fffcf5; margin: 20px; box-shadow: 0 0 20px rgba(0,0,0,0.1); font-family: 'Georgia', 'Times New Roman', serif;">
        <!-- Watermark -->
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); opacity: 0.1; font-size: 80px; font-weight: bold; white-space: nowrap; pointer-events: none;">${schoolName}</div>
        <div style="position: absolute; top: 20px; left: 20px; font-size: 12px; color: #999;">${serialNumber}</div>
        <div style="margin-bottom: 30px;">
          <h1 style="font-size: 48px; margin-bottom: 10px; color: #2f86eb;">${schoolName}</h1>
          <p style="font-size: 18px; font-style: italic;">${motto}</p>
          <hr style="width: 100px; border: 1px solid #2f86eb; margin: 10px auto;">
        </div>
        <div style="margin: 40px 0;">
          <h2 style="font-size: 32px; letter-spacing: 2px; margin-bottom: 20px;">CERTIFICATE OF COMPLETION</h2>
          <p style="font-size: 20px;">This is to certify that</p>
          <p style="font-size: 36px; font-weight: bold; margin: 20px 0; text-transform: uppercase;">${selectedStudent.name}</p>
          <p style="font-size: 20px;">has successfully completed the course</p>
          <p style="font-size: 28px; font-weight: bold; margin: 20px 0;">${selectedStudent.course}</p>
          <p style="font-size: 18px;">with effect from ${formattedCompletion}</p>
          <p style="font-size: 18px; margin-top: 20px;">In witness whereof, we have hereunto set our hand and seal this ${now.toLocaleDateString()}.</p>
        </div>
        <div style="margin-top: 60px; display: flex; justify-content: space-between; align-items: flex-end;">
          ...
          <div style="text-align: center;">
            ${stampImage ? `<img src="${stampImage}" style="width: 100px; height: auto; margin-bottom: 10px;" />` : '<div style="width: 100px; height: 80px; border-top: 1px dashed #999; margin-bottom: 10px;"></div>'}
            <p>Official Stamp</p>
          </div>
          <div style="text-align: center; width: 200px;">
            <div style="border-top: 1px solid #000; width: 180px; margin: 0 auto;"></div>
    ...
    `;
    // Skip the global wrapper (no extra header/footer/stamp)
